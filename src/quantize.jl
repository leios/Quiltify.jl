...
# parent node
function naive_bin!(parent, box::ColorBox)
    println(parent.n)
    box.n = 0

    pixel_set = Array{RGB}(undef, length(parent.pixels))
    pixel_set[:] .= RGB(0)
    ...
    for pixel in parent.pixels
        ...
        if in_box(pixel, box)
            ...
            box.clr += pixel
            pixel_count += 1
            pixel_set[pixel_count] = pixel
        end
    end

    println(box.n, '\t', box.extents, '\t', box.clr)

    if pixel_count > 0
        box.pixels = pixel_set[1:pixel_count]
    end 
...
function divide_octree!(box::ColorBox, level::Int; max_level = 10)
    println(level)
    ...
    for i = 1:2, j = 1:2, k = 1:2
        ...
        naive_bin!(box, children[index])
        println(children[index].clr)
        divide_octree!(children[index], level+1; max_level=max_level)
    end

    return box
end

function make_octree(img; max_level = 10)

    # Creation of the initial octree box
    box = ColorBox(sum(img)/(length(img)), [], vcat(img...), length(img),
                   ((0.0, 1.0),(0.0, 1.0),(0.0, 1.0)))

    divide_octree!(box, 1; max_level = max_level)
...
function quantize(img, colornum)
    img_out = copy(img)
    box = make_octree(img; max_level=2)
